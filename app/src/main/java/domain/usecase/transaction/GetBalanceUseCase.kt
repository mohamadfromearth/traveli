...
class GetBalanceUseCase(private val app: ApplicationClass, private val repo: TransactionRepository, private val prefManager: PrefManager) {
    operator fun invoke() = flow {
        emit(DataState.Loading)
        prefManager.authorize({
            emit(repo.getBalance())
        }) {
